...
class FullyAssocLiFoCache implements Cache {
    ...
    private int[] location_stack; // use stack?   
    ...
    FullyAssocLiFoCache(int cacheSize, int cacheLineSize) {
        ...
        location_stack = new int[CACHE_NB_LINES]; // init location_stack 
        // init for HashMaps?
    }
    ...
    public void flush(int[] ram, Status status) {
        ...
        for(int i = 0; i < CACHE_NB_LINES; i++) {
        	for(int j=0; j<CACHELINE_SZ; j++) {
        		ram[cache_loc_to_address.get(i)+j] = cache_storage[i][j];
        	}
        }
        
        //remove all data from cache
        //reset location stack //set all locations to be empty
        location_stack
